...
function build(manifest, languageModels){
  const dialogflowManifest = buildManifest(manifest),
...
function buildIntents(manifest, languageModels){
  ...
  entries(languageModels).forEach(([locale, model]) => {
    entries(model.intents || {}).forEach(([intentName, intentConfig]) => {
      ...
      aliases.map(alias => buildIntent(alias, intentConfig, locale))
             .forEach(outputIntents => assign(intents, outputIntents));
    });
  });
  return intents;
}
...
function buildIntent(intentName, intentConfig, locale){
  const intents     = {},
  ...
  languages.forEach(language => {
    const usersays = [];
    entries(intentConfig.get('slots', {})).forEach(([slotName, slotConfig], i) => {
      const example = {
        id: aguid(`${slotName}-${i}`),
        data: [],
        isTemplate: true,
